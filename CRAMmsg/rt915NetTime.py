...
class rt915NetTime(CRAMBaseMessage):
    MSG_LEN = 40
    MSG_ID = 915
    PART_COUNT = 0
    ...
    numNetTimes = 0
    def sendResponse(self, CRAM, receiptTime) -> 'rt915NetTime':

        responseMsg = rt915NetTime(rt915Consts.msgType.RESPONSE.value, self.weaponId, self.requestTime, receiptTime,
                                   responseTime=millisSinceMidnight())

        CRAM.sendCRAMmsg(responseMsg)
        rt915NetTime.numNetTimes += 1

        debugLog.log(constants.NettimeResponseSentToCRAM + str(rt915NetTime.numNetTimes))
        return responseMsg
